...
from train import generate_layer_configurations, filter_layer_configurations, get_nuclei_range
...
def Standardization(inputs_original):
    train_min = np.array([MIN_N, 2, -0.45])
    train_max = np.array([MAX_N, 7, 0.60])
    return (inputs_original - train_min) / (train_max - train_min)
...
def evaluate_model(inputs_original, inputs_scaled, expt, configs):
    ...
    for idx, config in enumerate(configs):
        ...
        for i in range(num_inputs):
            ...
            results = [outputs[0][0], outputs[0][1]]
            ...
            if count:
                ...
                total_count += count
                if energies[0] != 0 and not np.isnan(expt_data_N[0]) and not np.isnan(expt_data_N[1]):
        ...
        ratio_RMS = np.sqrt(ratio_RMS / ratio_count)
